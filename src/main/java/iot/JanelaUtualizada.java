 ...
 * @author cesar
 */
public class JanelaUtualizada extends javax.swing.JFrame {
    ...
    public JanelaUtualizada() {
        ...
        addWindowListener(new WindowListener() {
            ...
            @Override
            public void windowClosing(WindowEvent e) {// implementar este método para quando fechar a janela ele exercutar (salvarNoSerializable)
                inserirDadosNoSerializable(listaLocal);
                System.out.println("LISTA INSERIDA COM SUCESSO!!!!!!!!!!");
            }
...
//        listaDeTarefas.setModel(dlm);
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        try {
            rxtx.enviarDados(proxDado.getLuzDoQuarto()+proxDado.getLuzDaSala()+proxDado.getTv()+proxDado.getVentilador());
        } catch (IOException ex) {
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(JanelaUtualizada.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new JanelaUtualizada().setVisible(true);
            }
        });
    }
     ...
     * @param dado
     */
    public void inserirDadosNoSerializable(List<DadosCalendar> lista) {
//            List<DadosCalendar> listaAux = new ArrayList<DadosCalendar>();
//            List<DadosCalendar> listaSerInserida = new ArrayList<DadosCalendar>();
//            listaAux = buscarDadosNoSerializable();
//            int condicao = 0;
//            for (DadosCalendar local : lista) {
//                for (DadosCalendar l : listaAux) {
//                    if(local.getId() == l.getId()){
//                        condicao++;
//                    }
//                }
//                if(condicao > 0){
//                    listaSerInserida.add(local);
//                }
//            }
//            
            deletarOsDadosPassados(lista);// para não salvar nenhum da passado
            colocarIdNosDados(lista);
        try {
            serializador.gravarUmaListaDeDados(lista);
        } catch (IOException ex) {
            Logger.getLogger(JanelaUtualizada.class.getName()).log(Level.SEVERE, null, ex);
        }
...
//                JOptionPane.showMessageDialog(null, "CHEGOU HORÁRIO!!!!");
                System.out.println("CHEGOU HORÁRIO!!!!");
...
//            updateDados(proxDado);// depois da alteração da situação do proxDado devemos altera-lo no Serializable.
    ...
    public static void deletarOsDadosPassados(List<DadosCalendar> dados) {
...
//   
    ...
    private void conectarComWebSocket() {
        ...
        try {
            session = webSocketContainer.connectToServer(JanelaUtualizada.MyEndpoint.class,
    ...
    public static class MyEndpoint {// nesse caso é melhor que a classe 
        ...
        public void onMessage(String dado, Session session) { // toda vez que chegar uma msg do servidor isso será executado
            ...
            if (d.getSolicitacao().equals("ENVIA")) {
                JanelaUtualizada.mandarDadosParaWeb(listaLocal);
                return;

            }
            d.setDataHorario(gerenciadorCalendar.tranformarMilissegundoEmDate(d));
            if (d.getId() == 0) {
...
//                d.setDataHorario(new GerenciadorCalendar().novoAtualizarDataHorarioDoServidor(d));// ATUALIZANDO O CALENDAR
                JanelaUtualizada.chegouNovoAgendamento(d);
...
//                    d.setDataHorario(gerenciadorCalendar.tranformarMilissegundoEmDate(d));
//                }
                JanelaUtualizada.deleteOuUpdate(d);
                return;
            }
